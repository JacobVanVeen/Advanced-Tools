...
Renderer::Renderer()
{
   ...
   // "  gl_Position = vec4(vp, 1.0);"
    ...
    "uniform mat4 projectionMatrix;"
    "void main() {"
    "  vec3 lightDir = normalize(vec3(0,0,-12) - fragPos);"
    "  float diff = max(dot(normal,lightDir),0.0);"
  //  "  frag_colour = (0.1 + diff) * vec4(1, 0.0, 0, 1.0);"
    "  frag_colour = (0.1 + diff) * colorv + vec4(0.1,0.1,0.1,0.1);"
    "}";
...
void Renderer::Render(Gameobject* pObject,glm::mat4& pViewMat,glm::mat4& pProjectionMat)
{
    ...
    glUniform4fv (glGetUniformLocation (shader_programme, "colorv"),1,glm::value_ptr(pObject->Color));
